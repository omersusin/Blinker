...
class ExtensionManager(private val context: Context) {
    ...
    private val prefs = context.getSharedPreferences("blinker_extensions", Context.MODE_PRIVATE)
    ...
    fun getAll(): List<ExtensionInfo> {
        ...
        return try {
            ...
            (0 until array.length()).mapNotNull { i ->
                try { deserialize(array.getJSONObject(i)) } catch (_: Exception) { null }
            }
    ...
    fun getExtensionUrl(extensionId: String, path: String): String {
        return ExtensionUrlHandler.getUrl(extensionId, path)
    }

    private fun urlMatches(url: String, matches: List<String>, excludes: List<String>): Boolean {
    ...
    private fun patternMatches(url: String, pattern: String): Boolean {
        ...
        return try {
            val regex = pattern.replace(".", "\\.").replace("*://", "(https?|ftp)://").replace("*", ".*")
    ...
    private fun serialize(ext: ExtensionInfo) = JSONObject().apply {
        put("id", ext.id); put("name", ext.name); put("version", ext.version)
        put("description", ext.description); put("enabled", ext.enabled)
        put("type", ext.type.name); put("iconPath", ext.iconPath ?: "")
        put("optionsPage", ext.optionsPage ?: "")
        put("backgroundPage", ext.backgroundPage ?: "")
        put("backgroundScripts", JSONArray(ext.backgroundScripts))
        put("permissions", JSONArray(ext.permissions))
        put("content_scripts", JSONArray().apply {
            ext.contentScripts.forEach { cs -> put(JSONObject().apply {
                put("matches", JSONArray(cs.matches))
                put("exclude_matches", JSONArray(cs.excludeMatches))
                put("js", JSONArray(cs.js)); put("css", JSONArray(cs.css))
                put("run_at", cs.runAt)
            })}
        })
    }
    ...
    private fun deserialize(obj: JSONObject): ExtensionInfo {
        ...
        return ExtensionInfo(
            ...
            type = try { ExtensionType.valueOf(obj.optString("type", "CRX")) } catch (_: Exception) { ExtensionType.CRX },
            permissions = toList(obj.optJSONArray("permissions")),
            iconPath = obj.optString("iconPath", "").ifEmpty { null },
            optionsPage = obj.optString("optionsPage", "").ifEmpty { null },
            backgroundPage = obj.optString("backgroundPage", "").ifEmpty { null },
            backgroundScripts = toList(obj.optJSONArray("backgroundScripts")),
            contentScripts = (0 until csArray.length()).map { i ->
                ...
                ContentScript(
                    ...
                    js = toList(cs.optJSONArray("js")), css = toList(cs.optJSONArray("css")),
