...
object ExtensionParser {
    ...
    private fun loadMessages(manifest: JSONObject, extDir: File): Map<String, String> {
        ...
        for (locale in candidates) {
            ...
            if (f.exists()) { messagesFile = f; break }
        }
        ...
        return try {
            ...
            json.keys().forEach { key ->
                val msg = json.optJSONObject(key)?.optString("message", "") ?: ""
                if (msg.isNotEmpty()) map[key.lowercase()] = msg
            }
            map
    ...
    private fun resolveI18n(text: String, messages: Map<String, String>): String {
        ...
        return Regex("__MSG_(\\w+)__").replace(text) { match ->
            messages[match.groupValues[1].lowercase()] ?: match.value
        }
    }
    ...
    private fun findBestIcon(manifest: JSONObject, extDir: File): String? {
        ...
        if (iconsObj != null) {
            for (size in listOf("128", "96", "64", "48", "32", "16")) {
                ...
                if (path.isNotEmpty() && File(extDir, path).exists()) return path
            }
        ...
        if (action != null) {
            val ai = action.optJSONObject("default_icon")
            if (ai != null) {
                ai.keys().forEach { key ->
                    val path = ai.optString(key, "")
                    if (path.isNotEmpty() && File(extDir, path).exists()) return path
                }
            }
            val si = action.optString("default_icon", "")
            if (si.isNotEmpty() && File(extDir, si).exists()) return si
        }
        return null
    }

    private fun findOptionsPage(manifest: JSONObject, extDir: File): String? {
        val optUi = manifest.optJSONObject("options_ui")
        if (optUi != null) {
            val page = optUi.optString("page", "")
            if (page.isNotEmpty() && File(extDir, page).exists()) return page
        }
        val optPage = manifest.optString("options_page", "")
        if (optPage.isNotEmpty() && File(extDir, optPage).exists()) return optPage
        return null
    }

    private fun findBackgroundScripts(
        manifest: JSONObject, extDir: File
    ): Pair<List<String>, String?> {
        val bg = manifest.optJSONObject("background")
        if (bg != null) {
            // MV2: background.scripts
            val scripts = toList(bg.optJSONArray("scripts"))
                .filter { File(extDir, it).exists() }

            // MV2: background.page
            val page = bg.optString("page", "")
            val bgPage = if (page.isNotEmpty() && File(extDir, page).exists()) page else null

            // MV3: background.service_worker
            if (scripts.isEmpty() && bgPage == null) {
                val sw = bg.optString("service_worker", "")
                if (sw.isNotEmpty() && File(extDir, sw).exists()) {
                    return listOf(sw) to null
                }
            }

            return scripts to bgPage
        }
    ...
    private fun detectType(bytes: ByteArray): ExtensionType? {
        ...
        if (bytes[0] == 0x43.toByte() && bytes[1] == 0x72.toByte() &&
            bytes[2] == 0x32.toByte() && bytes[3] == 0x34.toByte()) return ExtensionType.CRX
        if (bytes[0] == 0x50.toByte() && bytes[1] == 0x4B.toByte()) return ExtensionType.XPI
        return null
    }
    ...
    private fun extractCrxZip(bytes: ByteArray): ByteArray? {
        ...
        return when (version) {
            2 -> {
                ...
                val pkLen = buf.int; val sigLen = buf.int
                val offset = 16 + pkLen + sigLen
                if (offset >= bytes.size) null else bytes.copyOfRange(offset, bytes.size)
            }
            3 -> {
                val hdrLen = buf.int
                val offset = 12 + hdrLen
                if (offset >= bytes.size) null else bytes.copyOfRange(offset, bytes.size)
            }
            else -> null
        }
    ...
    private fun buildInfo(
        manifest: JSONObject, id: String, type: ExtensionType,
        messages: Map<String, String>, extDir: File
    ): ExtensionInfo {
        val name = resolveI18n(manifest.optString("name", "Unknown Extension"), messages)
        val description = resolveI18n(manifest.optString("description", ""), messages)
        ...
        val (bgScripts, bgPage) = findBackgroundScripts(manifest, extDir)
        ...
        manifest.optJSONArray("content_scripts")?.let { csArray ->
            for (i in 0 until csArray.length()) {
                ...
                contentScripts.add(ContentScript(
        ...
        return ExtensionInfo(
            ...
            contentScripts = contentScripts, type = type,
            permissions = toList(manifest.optJSONArray("permissions")),
            iconPath = iconPath, optionsPage = optionsPage,
            backgroundScripts = bgScripts, backgroundPage = bgPage
        )
    }
